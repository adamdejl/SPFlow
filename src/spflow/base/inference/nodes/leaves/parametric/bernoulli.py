...
def node_likelihood(node: Bernoulli, data: np.ndarray) -> np.ndarray:
...
@dispatch(Bernoulli, data=np.ndarray)  # type: ignore[no-redef]
def node_log_likelihood(node: Bernoulli, data: np.ndarray) -> np.ndarray:
    probs = np.zeros((data.shape[0], 1))
    data = data[:, node.scope]
    marg_ids = np.isnan(data)
    probs[~marg_ids] = (
        get_scipy_object(node)
        .logpmf(k=data[~marg_ids], **get_scipy_object_parameters(node))
        .flatten()
    )
