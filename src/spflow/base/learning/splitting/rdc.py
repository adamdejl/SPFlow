...
from spflow.base.learning.splitting.base import clusters_by_adjacency_matrix, split_data_by_clusters
...
def get_RDC_transform(
    data: np.ndarray, meta_types: List[MetaType], ohe: bool = False, k: int = 10, s: float = 1 / 6
) -> np.ndarray:
    """<header>
...
def get_RDC_adjacency_matrix(data, meta_types, ohe=False, linear=True) -> np.ndarray:
    """<header>
...
def get_split_cols_RDC(threshold: float = 0.3, ohe: bool = True, linear: bool = True) -> Callable:
    """<header>
    ...
    def split_cols_RDC(
        ...
        adjm = get_RDC_adjacency_matrix(
            local_data, context.get_meta_types_by_scope(scope), ohe, linear
        )
...
def get_split_rows_RDC(
    n_clusters: int = 2, k: int = 10, s: float = 1 / 6, ohe: bool = True, seed: int = 17
) -> Callable:
    """<header>
    ...
    def split_rows_RDC(
        ...
        """
        data = get_RDC_transform(local_data, context.get_meta_types_by_scope(scope), ohe, k=k, s=s)
...
from networkx.algorithms.components.connected import connected_components  # type: ignore
from networkx.convert_matrix import from_numpy_array  # type: ignore
...
def rdc_transformer(
    ...
    if return_matrix:
        ...
        return [
            np.concatenate((f, np.ones((f.shape[0], 1))), axis=1) for f in nl_rand_proj_features
        ]
...
def get_split_cols_RDC_py(
    threshold=0.3, ohe=True, k=10, s=1 / 6, non_linearity=np.sin, n_jobs=-2, rand_gen=None
):
...
def get_split_rows_RDC_py(
    n_clusters=2, ohe=True, k=10, s=1 / 6, non_linearity=np.sin, n_jobs=-2, seed=None
):
    def split_rows_RDC_py(local_data, ds_context, scope):
        ...
        clusters = KMeans(n_clusters=n_clusters, random_state=seed).fit_predict(rdc_data)
